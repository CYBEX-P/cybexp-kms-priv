...
function print_help {
   echo
   echo 'Manipulate docker-compose to start and stop KMS and DB servives. By default the container are left behind,'
...
#       $DOCKER_COMPOSE up -d priv-kms-db
#    fi
#    echo hey
#    $DOCKER_COMPOSE run priv-kms-api --entrypoint bash
#    $DOCKER_COMPOSE stop
#    popd > /dev/null 2>&1 # supress output
